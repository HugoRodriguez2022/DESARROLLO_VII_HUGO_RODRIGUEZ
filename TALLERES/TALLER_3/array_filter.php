...
$palabras = ["auto", "casa", "elefante", "iglú", "oso", "uva", "zapato","aguila","arpa","espanto"];
...
// Función para filtrar personas menores de 25 años
$menoresDe25 = array_filter($personas, function($persona) {
    return $persona['edad'] < 25;
});

echo "</br>Personas menores de 25 años:</br>";
foreach ($menoresDe25 as $persona) {
    echo "- {$persona['nombre']} ({$persona['edad']} años)</br>";
}
...
$estudiantesPython = filtrarPor($estudiantes, "curso", "Python");
echo "</br>Estudiantes de Python:</br>";
foreach ($estudiantesPython as $estudiante) {
    echo "- {$estudiante['nombre']} (Nota: {$estudiante['nota']})</br>";
}

// Filtrar estudiantes con notas mayores o iguales a 90
$notasAltas = array_filter($estudiantes, function($estudiante) {
    return $estudiante['nota'] >= 90;
});

echo "</br>Estudiantes con notas mayores o iguales a 90:</br>";
foreach ($notasAltas as $estudiante) {
    echo "- {$estudiante['nombre']} (Nota: {$estudiante['nota']})</br>";
}
?>
