...
function DifdiasEntre($fecha1, $fecha2) {
    $timestamp1 = strtotime($fecha1);
    $timestamp2 = strtotime($fecha2);
    $diferencia = abs($timestamp2 - $timestamp1);
    return floor($diferencia / (60 * 60 * 24));
...
$fechaFinal = date("Y-m-d"); // Fecha actual
$diasPasados = diasEntre($fechaInicial, $fechaFinal);


echo "</br>Días transcurridos desde el $fechaInicial hasta hoy: $diasPasados días</br>";
